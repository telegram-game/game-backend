...
import {
  CreateMissionReqeust,
  MissionClaimRewardRequest,
} from '../models/mission';
...
export class MissionService {
    ...
    return await this.missionRepository
      ...
      .catch((error) => {
        if (error.code === 'P2002') {
          throw new BusinessException({
            status: HttpStatus.BAD_REQUEST,
            errorCode: 'DUPLICATE_MISSION_CODE',
            errorMessage: 'Mission code is duplicated',
          });
        }
        throw error;
      });
  ...
  async getsSatifiedByUserId(userId: string): Promise<Missions[]> {
    return this.missionRepository
      .getsSatifiedByUserId(userId)
      .then((missions) =>
        missions.filter(
          (mission) =>
            !mission.userMissions || mission.userMissions?.length === 0,
        ),
      );
  }
  ...
  async claimReward(
    userId: string,
    data: MissionClaimRewardRequest,
  ): Promise<void> {
    // TODO: Don't check the token first before we integrate with the wallet service

    const mission = await this.missionRepository.getSatifiedById(
      data.missionId,
    );
    if (!mission) {
      throw new BusinessException({
      ...
      await this.userMissionRepository
        ...
        .catch((error) => {
          if (error.code === 'P2002') {
            throw new BusinessException({
              status: HttpStatus.BAD_REQUEST,
              errorCode: 'DUPLICATE_MISSION_CLAIM',
              errorMessage: 'Mission is already claimed',
            });
          }
          throw error;
        });
      await this.missionRepository.updateCurrentTotalClaimedWithSatifyCondition(
        data.missionId,
        mission.metaData.rewardValue,
      );
