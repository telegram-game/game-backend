...
import {
  FullGameProfile,
  FullGameProfileRepositoryModel,
...
export class GameProfileService extends BaseGameMatchService {
  ...
  async getRandomSameLevelGameProfile(
    ...
    const gameProfile: FullGameProfileRepositoryModel =
      await this.gameProfileRepository.getRandomSameLevelGameProfile(
        fromLevel,
        toLevel,
        {
    ...
    Object.keys(UserGameProfileAttribute).forEach((key) => {
      const attribute =
        UserGameProfileAttribute[key as keyof typeof UserGameProfileAttribute];
      const userGameProfileAttribute =
        gameProfile.userGameProfileAttributes?.find(
      ...
      attributes[attribute] = {
        ...
        cost: this.calculateUpgradeCost(
          attribute,
          userGameProfileAttribute?.value || 1,
        ),
    ...
    return {
      ...
      currentGameProfileSeason: this.getGameProfileSeason(
  ...
  async createOrGetFullFirst(userId: string): Promise<FullGameProfile> {
    ...
    let gameProfile: FullGameProfileRepositoryModel =
      await this.gameProfileRepository.getFirst(userId, {
        includeAttributes: true,
        includeGameSeasonId,
      });
  ...
  async upgradeAttribute(
    ...
    const upgradeInformation =
      configurationData.system.upgradeInformation[attribute];
    ...
    if (!gameProfile) {
      throw new BusinessException({
        status: HttpStatus.BAD_REQUEST,
        errorCode: 'GAME_PROFILE_NOT_FOUND',
        errorMessage: 'Game profile not found',
      });
    }

    const attr = gameProfile.userGameProfileAttributes.find(
    ...
    const repositories = [
      this.gameProfileAttributeRepository,
      this.gameProfileRepository,
    ];
    await this.prismaService.transaction(async () => {
      ...
      const totalLevel =
        await this.gameProfileAttributeRepository.getTotalLevel(
          userId,
          gameProfileId,
        );
  ...
  private calculateUpgradeCost(
    ...
    return (
      upgradeInformation.baseCost *
      Math.pow(current, upgradeInformation.multiplier)
    );
  }
}
