...
export class TelegramBusinessService implements OnModuleInit {
  ...
  private async onSuccessfulPayment(ctx: any) {
    ...
    const { chestCode, userId, gameProfileId } = JSON.parse(
    ...
    await this.inventoryService
      .openChestExecution(chestCode, userId, gameProfileId, {
        provider: 'TELEGRAM',
      ...
      .catch((err) => {
        ...
        if (
          err?.code === 'P2002' &&
          err?.meta?.modelName === 'UserGameInventories'
        ) {
          this.logger.info(
            "Duplicate data. It's mean the user already has the item",
          );
        ...
        this.telegramBotService
          .refund(ctx.from.id, telegramPaymentChargeId)
          .catch((err) => {
