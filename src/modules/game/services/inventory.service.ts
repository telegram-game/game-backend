...
export class InventoryService extends BaseInventoryService {
  ...
  async openChest(
    ...
    gameProfileId = gameProfileId ?? (await this.gameProfileService.getByIdOrFirst(userId)).id;
    ...
    const fixedAttributes = this.supportService.randomWithList<ItemAttributeData>(attributes.fixedItemAttributes, chest.fixedItemAttributesCount, true);
    const flexibleItemAttributes = this.supportService.randomWithList<ItemAttributeData>(attributes.flexibleItemAttributes, chest.flexibleItemAttributesCount, true);
    console.log(`Hoang ${itemType}, ${itemCode}, ${star}`, attributes, fixedAttributes,flexibleItemAttributes )
    await this.prismaService.$transaction(async (tx: PrismaService) => {
      ...
      for (const attribute of fixedAttributes) {
        const star = parseInt(this.supportService.randomWithRate(attribute.starRate));
        const value = {
          point: this.supportService.buildValue(attribute.value.point, star),
          percent: this.supportService.buildValue(attribute.value.percent, star),
          percentPerTime: this.supportService.buildValue(attribute.value.percentPerTime, star)
        }
