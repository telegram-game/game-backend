...
export class GameProfileAttributeRepository extends BaseRepository {
  ...
  async create(
    data: Partial<UserGameProfileAttributes>,
  ): Promise<UserGameProfileAttributes> {
    return this.client.userGameProfileAttributes.create({
      data: data as UserGameProfileAttributes,
    });
  }

  async updateOptimstic(
    data: Partial<UserGameProfileAttributes>,
    updatedAt: Date,
  ): Promise<UserGameProfileAttributes> {
  ...
  async getTotalLevel(userId: string, gameprofileId: string): Promise<number> {
    return this.client.userGameProfileAttributes
      .aggregate({
        where: {
          userId,
      ...
      .then(
        (result) =>
          result._sum.value ?? Object.keys(UserGameProfileAttribute).length,
      );
  }
}
